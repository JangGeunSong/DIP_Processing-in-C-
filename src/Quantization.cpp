...
unsigned char** _2dQuantization(unsigned char** originaImg) {
    ...
    for(x = 0; x < 512; x++) {
        for(y = 0; y < 512; y++) {
            quantValue[x][y] = (int)round(values[x][y] / quntNum);
        }
    }
    ...
    for(x = 0; x < 512; x++) {
        for(y = 0; y < 512; y++) {
            quantValue[x][y] = quantValue[x][y] * quntNum;
        }
    }
    ...
    //                         valueCos_1 = cos((PI * ((2 * (double)m) + 1) * (double)u) / 16.0);
    //                         valueCos_2 = cos((PI * ((2 * (double)n) + 1) * (double)v) / 16.0);
    //                         IDCT[(x * 8) + m][(y * 8) + n] = IDCT[(x * 8) + m][(y * 8) + n] + (alpha_u * alpha_v * quantValue[(x * 8) + u][(y * 8) + v] * valueCos_1 * valueCos_2); 
    //                     }
    //                 }
    //             }
    //         }
    ...
    for(m = 0; m < 512; m++) {
        for(n = 0; n < 512; n++) {
            ...
            else if(quantValue[m][n] > 256) {
                result[m][n] = 255;
            }
            else {
                result[m][n] = quantValue[m][n];
            }
        }
    }
...
int main() {
	...
	unsigned char** ppOutputImg = _2dQuantization(ppLena);
	printf("2dDCTQuantization is complete!");

	FILE* hOutput = fopen("8_DCT_Quantization.img", "wb");
