...
unsigned char** _2dDFT(unsigned char** originaImg) {
	...
	int m = 0, n = 0;
	int u = 0, v = 0, mnSum = 1, distance = 0, D_0 = 0;
	double coef = 0, res = 0, totalPixel = 128 * 128, gaussValue = 0; //DCvalue (0,0)의 값.
    ...
    //         m = u - 64;
    //         n = v - 64;
    //         if(sqrt(m*m + n*n) > distance) {
    //             dftValues[u][v].imag = 0;
    //             dftValues[u][v].real = 0;
    //         }
    //     }
    // }
    // printf("Ideal LPF passed\n");
    ...
    //         n = v - 64;
    //         D_0 = sqrt(m*m + n*n);
    //         gaussValue = exp(-((D_0 * D_0) / (2.0 * (double)distance * (double)distance)));
    //         dftValues[u][v].real = dftValues[u][v].real * gaussValue;
    //         dftValues[u][v].imag = dftValues[u][v].imag * gaussValue;
    //     }
    // }
    // printf("Gaussian LPF passed\n");
    ...
    for(u = 0; u < 128; u++) {
        for(v = 0; v < 128; v++) {
            ...
            n = v - 64;
            D_0 = sqrt(m*m + n*n);
            gaussValue = 1 - exp(-((D_0 * D_0) / (2.0 * (double)distance * (double)distance)));
            dftValues[u][v].real = dftValues[u][v].real * gaussValue;
            dftValues[u][v].imag = dftValues[u][v].imag * gaussValue;
        }
    }
    printf("Gaussian HPF passed\n");
...
int main() {
	...
	printf("2dIDFT passfiltering is complete!");

	FILE* hOutput = fopen("passFilteringGaussHPF.img", "wb");
